...
#   cargo install cargo-deb            # only needed for `deb`
# One-time setup on the Pi (headers, .pc files, crt/libgcc objects):
#   sudo apt install libavformat-dev libavcodec-dev libavutil-dev \
#       libswresample-dev libasound2-dev libgcc-14-dev
#
# Usage:
#   ./build-pi.sh sync <pi-host>   (re)create the sysroot from the Pi
#   ./build-pi.sh build            cross-compile the release binary
#   ./build-pi.sh deb              build radiod_<version>_armhf.deb
#
...
sync)
    ...
    # (The Pi's LTO plugin is excluded — the host linker would try to
    # dlopen it, and it is 32-bit ARM code.)
...
*)
    echo "usage: $0 [sync <pi-host> | build | deb]" >&2
    exit 2
    ;;
esac
